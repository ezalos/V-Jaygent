...
boundary an infinite bead-chain. Drive integer `n` from the music
(level pumps it, each downbeat snaps it up briefly): the rosette
unfolds petals through the build and collapses on the drop. The
keyboard overrides `n` — each of 15 keys plays a specific n value
(3 to 17), so the synth is a fractal-shape sequencer.

Third piece in the basin/fractal series — see
`brainstorming/inspirations/rosette-refs.md`.
