...
// [-aspect/2, aspect/2] × [-0.5, 0.5]). 0.32 fills a comfortable
// portion of the canvas without crowding the corners.
const float R_BALL  = 0.32;

// Hex-lattice spike-spacing factor — same trick as before, gives the
// ferrofluid its canonical hex-cone signature where the field is high.
const float HEX_PERIOD = 0.038;
...
// Tight Gaussian magnet pull — fingers create a localized field that
// pulls fluid up into spikes. Range falls off sharply so each finger's
// effect is visible without dragging the whole ball.
float magnetPull(vec2 p, vec2 src, float strength) {
    vec2  d  = p - src;
    float r2 = dot(d, d);
    return strength * exp(-r2 * 95.0);
}

// Rim activity factor — the displacement field h matters most NEAR the
// sphere boundary (within a thin annulus). Multiplying forcing by this
// factor concentrates spike formation on the visible silhouette so the
// ball appears to grow spikes from its surface, not deep inside.
float rimBand(float r) {
    // Active in [R_BALL - 0.06, R_BALL + 0.18]; outside this range the
    // sim is essentially quiescent.
    float inner = smoothstep(R_BALL - 0.10, R_BALL - 0.02, r);
    float outer = 1.0 - smoothstep(R_BALL + 0.05, R_BALL + 0.20, r);
    return inner * outer;
}
...
void main() {
    ...
    // Three orbiters traveling JUST OUTSIDE the sphere boundary act as
    // phantom fingers — pulling spikes outward all around the ball over
    // time. So inspect frames show the levitating-ball-with-spikes look
    // even without input.
    if (u_touch_count == 0) {
        float t  = u_time;
        float Ro = R_BALL + 0.06;     // orbit radius — just outside the ball
        vec2 m1 = Ro * vec2(cos(t * 0.42),       sin(t * 0.42 + 0.0));
        vec2 m2 = Ro * vec2(cos(t * 0.31 + 2.4), sin(t * 0.31 + 2.4));
        vec2 m3 = Ro * vec2(cos(t * 0.27 + 4.7), sin(t * 0.27 + 4.7));
        E      += magnetPull(p, m1, 1.30);
        E      += magnetPull(p, m2, 1.10);
        E      += magnetPull(p, m3, 0.95);
    ...
    // magnet (by minDist) AND above critical field strength.
    float spikeProximity = exp(-minDist * 14.0);
