...
void main() {
    ...
    // master clock: audio time when playing, else u_frame wall-clock (u_time
    // freezes with no audio under time_source:audio — keeps the pool drifting
    // in the idle cell).
    float CK = (playing > 0.5) ? u_time : float(u_frame) * 0.01667;
    float bass = mix(0.30 + 0.22 * sin(CK * 0.7), u_audio_bass_stem + 0.4 * u_audio_bass, playing);
    ...
    vec2 hot = vec2(0.34 * sin(CK * 0.061), 0.26 * sin(CK * 0.043 + 1.3)) + secOff;
    ...
    vec2 hotB = vec2(0.30 * sin(CK * 0.037 + 2.1), 0.34 * sin(CK * 0.029 + 0.4)) - 0.7 * secOff;
    float r1 = 0.16 * (1.0 + 0.3 * bass);
    float core2 = 0.55 * exp(-pow(length(p - hotB) / r1, 2.0));

    float energy = mix(0.30 + 0.15 * sin(CK * 0.2), u_energy_smooth, playing);
